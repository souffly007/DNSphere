...
// the Free Software Foundation, either version 3 of the License.class Profilesactivity {
...
class ProfilesActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        // Bouton global Planification (tous les profils)
        findViewById<View>(R.id.btnSchedule)?.setOnClickListener {
            startActivity(Intent(this, ProfileScheduleActivity::class.java))
        }
    ...
    private fun showProfileMenu(profile: Profile, anchor: View) {
        ...
        if (!profile.isActive) popup.menu.add(0, 1, 0, R.string.profile_activate)
        if (!profile.isPreset) {
        ...
        // Option planification disponible pour tous les profils
        popup.menu.add(0, 5, 0, "⏰ Planifier")
        ...
        popup.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                ...
                5 -> openScheduleForProfile(profile)
            }
            true
        }
        popup.show()
    ...
    private fun openScheduleForProfile(profile: Profile) {
        val intent = Intent(this, ProfileScheduleActivity::class.java).apply {
    ...
    private fun showAddDialog() { showProfileDialog(null) }
    private fun showEditDialog(profile: Profile) { showProfileDialog(profile) }
    ...
    private fun saveProfile(profile: Profile, isNew: Boolean) {
        lifecycleScope.launch {
            if (isNew) {
                ...
                if (profile.isActive) updateVpnService(profile)
            }
        }
    }
    ...
    private fun duplicateProfile(profile: Profile) {
        val newProfile = profile.copy(
            id = 0, name = "${profile.name} (copie)", isPreset = false, isActive = false
        )
        lifecycleScope.launch {
